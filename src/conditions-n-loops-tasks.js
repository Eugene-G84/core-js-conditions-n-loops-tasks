...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < matrix.length - i; j += 1) {
      const temp = res[i][j];
      res[i][j] = res[matrix.length - 1 - j][res.length - 1 - i];
      res[res.length - 1 - j][res.length - 1 - i] = temp;
    }
  }
  ...
  for (let i = 0; i < matrix.length / 2; i += 1) {
    for (let j = 0; j < matrix.length; j += 1) {
      const temp = res[i][j];
      res[i][j] = res[matrix.length - 1 - i][j];
      res[matrix.length - 1 - i][j] = temp;
    }
  }
...
function sortByAsc(arr) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    if (arr[0] > arr[i]) left[left.length] = arr[i];
    else right[right.length] = arr[i];
  }

  const temp = [...sortByAsc(left), arr[0], ...sortByAsc(right)];
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations % 36; i += 1) {
    let even = '';
    let odd = '';
...
function getNearestBigger(number) {
  const arr = Array.from(String(number), (elem) => Number(elem));
  ...
  for (let i = arr.length; i > 0; i -= 1) {
    if (arr[i] > arr[i - 1]) {
      ...
      [arr[i - 1], arr[current]] = [arr[current], arr[i - 1]];
      const tail = arr.splice(i).sort();
      return Number([...arr, ...tail].join(''));
    }
  }
